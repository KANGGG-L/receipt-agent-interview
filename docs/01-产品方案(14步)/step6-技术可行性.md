...
2. **多模态 VLM 直识（L4）确立为当前生产冠军路径**：无需经过 OCR 文本流的中间语义损失，大模型直接在像素空间端到端理解“行文空间关系、印章遮挡、划线修改”，单据解析成功率达到 100%，平均处理耗时仅 9.0 秒，成本可控（<HK$0.02/张）。
...
│                                                                                        │
│   【正常主链路（L4 冠军路径）】                                                        │
│    手机拍照 ──► Qwen3-VL-Flash / CodeBuddy 主引擎 ──► 算术门禁 ──► 左右分栏秒级核对    │
│                                                                                        │
│                                  │ （若遇超时 15s 或解析异常）                          │
│                                  ▼                                                     │
│   【一级自动降级（重试与备用引擎）】                                                    │
