...
- **现状**：L4 多模态 VLM 直识为定案冠军（163/163 成功、P50 9.0s、约 HK$0.0024/张）；成本路由只体现在"是否跑审核腿"（`audit_mode: text|vlm|ondemand`）与"是否跑 parse LLM"，**没有按单据复杂度分流的实际执行**。L3 商业云 OCR 仍是"最重要缺项"未测。
