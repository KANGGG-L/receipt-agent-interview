...
def test_mock_judge_does_not_peek_expected_verdict():
    """规则 judge 不得偷看 expected_verdict（否则元评测自证）。"""
    import inspect
    src = inspect.getsource(meta_eval.mock_judge)
    assert "expected_verdict" not in src, "mock judge 读取了 expected_verdict，元评测失去意义"
