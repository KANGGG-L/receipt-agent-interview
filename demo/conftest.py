# -*- coding: utf-8 -*-
"""demo/ 顶层夹具：测试 DB 隔离、`--run-eval` 开关与 eval 标记。
...
DB 隔离（P17 治本）：`_isolate_test_db` 为 autouse 夹具，未显式指定 DB_PATH 的
用例一律改用本轮测试专属的临时库，杜绝「裸跑 pytest 就往 live 库
demo/receipt_demo.db 插假单据」的历史污染（详见该夹具 docstring）。
...
图片落盘隔离（N2）：同一个 autouse 夹具还会把图片输出目录（原图 / 预处理产物 /
凭证）一并重定向到 tmp_path。why：此前只隔离了 DB，上传端点仍把文件写进模块级
`UPLOAD_DIR`（默认即 live 目录 demo/uploads），于是每跑一次根套件就留下无任何
单据引用的孤儿文件（如 `<id>.jpg` / `<id>_prep.jpg`）。详见夹具内注释。
"""
...
# 图片输出目录 = live 目录。用于判断某模块的落盘常量是否仍指向 live，只在
# 「仍是 live」时改写，尊重用例自身的隔离设置。
_LIVE_UPLOAD_DIR = os.path.abspath(os.path.join(os.path.dirname(os.path.abspath(__file__)), "uploads"))
...
def _redirect_image_output_dirs(monkeypatch, tmp_path):
    ...
    `main.UPLOAD_DIR`），它们的默认值就是 live 目录 demo/uploads。DB 已隔离时
    这些文件没有任何单据引用，却仍留在 live 目录，日积月累成为孤儿文件。
...
def _isolate_test_db(tmp_path, monkeypatch, request):
    ...
    why：本项目测试长期没有代码级隔离，conftest 从不兜底，只靠个别用例手写
    夹具；于是「裸跑一次 pytest demo/tests/」就会向 live 库
    demo/receipt_demo.db 插入数十条假单据并污染真实业务数据。
    ...
    进程级 registry 只在「用例执行期间 db.DB_PATH 被改成了别的库」时才登记归属
    （见夹具收尾的 `_record_module_local`）：模块级 env 声明的环境库在 setup 时就
    已经在位、不会被登记，因此「环境库全局共享」的既有行为完全不受影响。

    还原时只把 db.DB_PATH 改回并重建引擎；create_all 走 CREATE TABLE IF NOT
    EXISTS，且还原目标就是 live 库自身的 schema，不会改动 live 库数据。
    ...
    live_db_path = os.path.abspath(_db._default_db_path)
    ...
    if env_db_path in (os.path.abspath("./receipt_demo.db"), os.path.abspath("receipt_demo.db")):
        ...
        env_db_path = live_db_path

    entry_path = os.path.abspath(str(_db.DB_PATH))
    kind, source = _classify_db_path(
        entry_path, live_db_path, env_db_path, module_key, _MODULE_LOCAL_DB)
    ...
    env_is_sane = bool(raw_env) and bool(env_db_path) and env_db_path != live_db_path \
